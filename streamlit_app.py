...
def create_comparison_chart(strategy_obj, strategy_params, selected_instrument, start_date, end_date, main_portfolio):
    ...
    main_code = instruments.get(selected_instrument, selected_instrument)
    main_name = commodity_names.get(main_code, selected_instrument)
    ...
    fig.add_trace(go.Scatter(
        ...
        mode='lines',
        name=f'{main_name}',
        line=dict(color='#055c50', width=4),
        hovertemplate=f'{main_name}<br>Performance: %{{y:.2f}}%<extra></extra>'
    ))
    ...
    if comparison_commodities:
        for i, commodity_code in enumerate(comparison_commodities):
            try:
                ...
                comp_cum_returns = (1 + comp_portfolio['returns']).cumprod()
                comp_perf_pct = (comp_cum_returns - 1) * 100

                name = commodity_names.get(commodity_code, commodity_code)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate=f'{name}<br>Performance: %{{y:.2f}}%<extra></extra>'
                ))
